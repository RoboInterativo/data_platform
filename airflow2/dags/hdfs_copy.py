...
def write_dashboard ( **kwargs):
    ...
    table_list = ti.xcom_pull(task_ids='gettables', key='tables_list')
    print(table_list)
    #task_ids='push_task'
...
with DAG('HDFS_COPY', start_date=days_ago(1), schedule_interval='@daily') as dag:
    ...
    # write2dashboard=PythonOperator(
    #         task_id="write2dashboard", python_callable=write_dashboard, provide_context=True
    # )
    # load_table=PythonOperator(
    #                 task_id="load_table", python_callable=load_table, provide_context=True
    # )
    get_record_size=PythonOperator(
                    task_id="get_record_size", python_callable=get_record_size, provide_context=True
    )
